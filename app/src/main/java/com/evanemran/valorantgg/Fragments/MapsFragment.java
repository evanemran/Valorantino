...
public class MapsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        manager.GetMaps(agentResponseListener);
        return view;
    }

    private final ApiResponseListener<List<MapsResponse>> agentResponseListener = new ApiResponseListener<List<MapsResponse>>() {
        @Override
        public void didFetch(List<MapsResponse> response, String message) {
            recycler_maps.setHasFixedSize(true);
            recycler_maps.setLayoutManager(new GridLayoutManager(getContext(), 2));
            mapsAdapter = new MapsAdapter(getContext(), response, clickListener);
            recycler_maps.setAdapter(mapsAdapter);
    ...
    private final ClickListener<MapsResponse> clickListener = new ClickListener<MapsResponse>() {
        @Override
        public void onClicked(MapsResponse item) {
